    ...
    <div class="bg-gray-100">
        <form class="max-w-4xl mx-auto p-4" method="POST" action="{{ route('quiz.update', $quiz->id) }}" id="quizForm">
            ...
            <div class="bg-white p-4 rounded-lg shadow-md">
                ...
                <div class="space-y-4" id="questions-wrapper">
                    @foreach ($quiz->questions as $question)
                        <div class="p-4 bg-gray-100 rounded-lg question" id="question{{ $loop->index + 1 }}">
                            <button type="button" onclick="deleteQuestion({{ $loop->index + 1 }})"
                                class="text-red-500 cursor-pointer mb-4">
                            ...
                            <input type="text" value="{{ $question->content }}"
                                name="questions[{{ $loop->index + 1 }}][text]" placeholder="Tuliskan soal..."
                            ...
                            <div class="flex items-center mb-2">
                                ...
                                <select class="ml-auto p-2 border rounded-lg"
                                    name="questions[{{ $loop->index + 1 }}][question_type]">
                            ...
                            <div class="space-y-2">
                                @foreach ($question->answers as $answer)
                                    <div class="flex items-center answer" id="answer{{ $loop->parent->index + 1 }}a{{ $loop->index }}">
                                        <input type="hidden"
                                            name="questions[{{ $loop->parent->index + 1 }}][answers][{{ $loop->index }}][id]"
                                            value="{{ $answer->id }}">
                                        <input type="text" placeholder="Opsi" class="w-full p-2 border rounded-lg"
                                            value="{{ $answer->content }}"
                                            name="questions[{{ $loop->parent->index + 1 }}][answers][{{ $loop->index }}][text]"
                                            required>
                                        @error('questions.{{ $loop->parent->index + 1 }}.answers.{{ $loop->index }}.text')
                                            <span class="text-sm text-red-500 block">{{ $message }}</span>
                                        @enderror
                                        <input type="checkbox" class="ml-2"
                                            name="questions[{{ $loop->parent->index + 1 }}][answers][{{ $loop->index }}][is_correct]"
                                            {{ $answer->is_correct ? 'checked' : '' }}>
                                        <button type="button" onclick="deleteAnswer({{ $loop->parent->index + 1 }}, {{ $loop->index }})"
                                            class="text-red-500 cursor-pointer mb-4">
            ...
            <div class="flex justify-end mt-4 space-x-2">
                <button class="px-4 py-2 text-white bg-blue-500 rounded-lg disabled:bg-blue-200" name="action"
        ...
        document.getElementById('quizForm').addEventListener('submit', function(event) {
            ...
            xhr.upload.onprogress = function(event) {
                ...
                if (event.lengthComputable) {
                    ...
                    progressElement.value = percentComplete; // Update progress value
                    progressElement.textContent =
                        `${Math.round(percentComplete)}%`; // Optionally display percentage text
                }
            };
            ...
            xhr.onload = function() {
                const progressElement = document.getElementById('uploadProgress');
                progressElement.value = 100; // Ensure progress reaches 100% on completion
